...
#define NUMSIMON2 2 //Nombre de postes que s'iluminen per a superar la prova2
#define NUMSIMON4 4 //Nombre de postes que s'iluminen per a superar la prova4
#define NUMSIMON5 5 //Nombre de postes del simon final (ha de ser major o igual que NUMSIMON4 per a que funcione correctament)

int tempsEspera1=2000; //Temps d'espera davant dels sensors per tal de desactivar-los
int tempsEspera2=5000; //Temps de visualització dels estats de transició
int tempsEspera3=2000; //Temps de visualització de les postes encertades prova2, prova4 i prova5
...
int estatPostaVar[NUMSIMON5]; // Array per realitzar les proves 4 i 5
...
void prova1() {
  ...
  if (estatPosta1 == 0 && primeraPosta) {
    enciendeTODASPOSTAS(0,0,255); //BLAU
    primeraPosta=false;
  }
  //INICI SENSOR POSTA1
   if (estatPosta1 == 0 || estatPosta1 == 1 ){
    ...
    if (estatPosta1 == 0 && distancia1 >= 35 && distancia1 <=65 && (millis() - marcaTemps11) > 600 ){    
       if (DEBUG) {
    ...
    }else if (estatPosta1 == 1 && distancia1 >= 35 && distancia1 <=65 ) {
      if (DEBUG) {
   ...
   //CODI PER ENCENDRE I JUGAR AMB 5 POSTES   
    if (estatPosta1 == 2) {
      if (estatPosta2 == 0 || estatPosta2 == 1 ){
        long distancia2 = sonar[1].convert_cm(sonar[1].ping_median(6)); 
        //long distancia2=0;
        //long distanciaInst2=sonar[1].convert_cm(sonar[1].ping_median(6));  
        //miDistancia2.insertar(distanciaInst2);
              
        //if (miDistancia2.valida()){
        //  distancia2 = miDistancia2.media();
        //}
         if (DEBUG) {
        ...
        if (estatPosta2 == 0 && distancia2 >= 35 && distancia2 <=65 && (millis() - marcaTemps21) > 600){
          enciendeSoloPOSTA2(255,255,0);
          marcaTemps2 = millis();
          estatPosta2=1;
        }else if (estatPosta2 == 1 && distancia2 >= 35 && distancia2 <=65 ) {
          if ( (millis() - marcaTemps2) > tempsEspera1 ){
            client.publish("sala1/completado","on");
            enciendeSoloPOSTA2(0,255,0); //VERD
            estatPosta2=2;
          }
        }else if (estatPosta2 == 1 && (millis() - marcaTemps2) > 600){
          marcaTemps21=millis();
          enciendeSoloPOSTA2(0,0,255);
          estatPosta2=0;
        }
      }
      ...
      if (estatPosta2 == 2) {
        if (estatPosta3 == 0 || estatPosta3 == 1 ){
          ...
          //long distancia3=0;
          //miDistancia3.insertar(sonar[2].convert_cm(sonar[2].ping_median(4)));          
          //if (miDistancia3.valida()){
          //  distancia3 = miDistancia3.media();
          //}
           if (DEBUG) {
          ...
          if (estatPosta3 == 0 && distancia3 >= 35 && distancia3 <=65 && (millis() - marcaTemps31) > 600){
            enciendeSoloPOSTA3(255,255,0); //GROC
            marcaTemps3 = millis();
            estatPosta3=1;
          }else if (estatPosta3 == 1 && distancia3 >= 35 && distancia3 <=65 ) {
        ...
        if (estatPosta3 == 2) {
          if (estatPosta4 == 0 || estatPosta4 == 1 ){
            long distancia4=sonar[3].convert_cm(sonar[3].ping_median(6));
          //long distancia4=0;
          //miDistancia4.insertar(sonar[3].convert_cm(sonar[3].ping_median(4)));          
          //if (miDistancia4.valida()){
          //  distancia4 = miDistancia4.media();
          //}        
           if (DEBUG) {
          ...
          if (estatPosta4 == 0 && distancia4 >= 35 && distancia4 <= 65 && (millis() - marcaTemps41) > 600 ){
            enciendeSoloPOSTA4(255,255,0);
            marcaTemps4 = millis();
            estatPosta4=1;
          }else if (estatPosta4 == 1 && distancia4 >= 35 && distancia4 <= 65 ) {
            if ( millis() - marcaTemps4 > tempsEspera1 ){
              enciendeSoloPOSTA4(0,255,0);
              estatPosta4=2;
            }
          }else if (estatPosta4 == 1 && (millis() - marcaTemps4) > 600){
            enciendeSoloPOSTA4(0,0,255);
            estatPosta4=0;
            marcaTemps41 = millis();
          }
          }
          if (estatPosta4 == 2){
            ...
            //long distancia5=0;
            //miDistancia5.insertar(sonar[4].convert_cm(sonar[4].ping_median(4)));            
            //if (miDistancia5.valida()){
            //  distancia5 = miDistancia5.media();
            //}
             if (DEBUG) {
            ...
            if (estatPosta5 == 0 && distancia5 >= 35 && distancia5 <=65 && (millis() - marcaTemps51) > 600 ){
              enciendeSoloPOSTA5(255,255,0); //GROC
              marcaTemps5 = millis();
              estatPosta5=1;
            }else if (estatPosta5 == 1 && distancia5 >= 35 && distancia5 <= 65 ) {
    ...
    if (estatPosta1 == 2 && estatPosta2 == 2 && estatPosta3 == 2 && estatPosta4 == 2 && estatPosta5 == 2) {
...
void transicio1(){
  ...
  inicialitzaEstatPosta();
  inicialitzaArrayPostes();
  enciendeTODASPOSTAS(255,255,255);
  enciendeSoloPOSTA1(0,255,0);
  enciendeSoloCASCO(255,255,255);
  delay(tempsEspera2);
}

//Funcio per a realitzar la prova2. SIMON presencial de NUMSIMON2 postes a una distancia de entre 30 i 50 cms
void prova2(){
  if (DEBUG2) {
  ...
  if (estatPosta[index] == 0 && primeraPosta) {
    enciendePOSTA(arrayPostes[index],0,0,255); //BLAU
    primeraPosta=false;
  }
  //INICI SENSORS
  if (estatPosta[index] == 0 || estatPosta[index] == 1 ){
    //long distancia=0;    
    long distancia = sonar[arrayPostes[index]-1].convert_cm(sonar[arrayPostes[index]-1].ping_median(6));
    // miDistancia.insertar(sonar[arrayPostes[index]-1].convert_cm(sonar[arrayPostes[index]-1].ping_median(4)));    
    //if (miDistancia.valida()){
    //  distancia = miDistancia.media();    
    //}  
    if (DEBUG) {
      Serial.print(F("SENSOR : "));
      Serial.print(index);
      Serial.print(F("ARRAYPOSTES[index]"));
      Serial.print(arrayPostes[index]);
      Serial.print(F("DISTANCIA :"));
      Serial.print(distancia);
      Serial.println(F(" cms"));
    } 
    if (estatPosta[index] == 0 && distancia >= 35 && distancia <= 65 && (millis() - marcaTemps11) > 600){    
       if (DEBUG) {
          Serial.println(F("SENSOR : Primera Part -- estatPosta1=0"));          
        }
      enciendePOSTA(arrayPostes[index],255,255,0); //GROC  
      marcaTemps = millis();
      estatPosta[index]=1;      
      
    }else if (estatPosta[index] == 1 && distancia >= 35 && distancia <= 65 ) {
      if (DEBUG) {
      ...
      if ( (millis() - marcaTemps) > tempsEspera1 ){
        ...
        enciendePOSTA(arrayPostes[index],0,255,0); //VERD
        delay(tempsEspera3);
        estatPosta[index]=2;
        index++;        
        marcaTemps=0;
        marcaTemps11=0;
        primeraPosta=true;
        //miDistancia.inicializa();
      }
    }else if (estatPosta[index] == 1 && (millis() - marcaTemps) > 600){
      if (DEBUG) {
...
void prova3() {
  ...
  if (estatPosta1 == 0 && primeraPosta) {
    apagaCASCO();
    enciendeSoloPOSTA1(0,255,255); //MORADO 65-95
    enciendeSoloPOSTA2(0,0,255); //AZUL 35-95
    enciendeSoloPOSTA3(255,0,255); //BLAVET 5-35
    enciendeSoloPOSTA4(0,0,255); //AZUL 35-65
    enciendeSoloPOSTA5(0,255,255); //MORADO 65-95
  ...
  //INICI SENSOR POSTA1
   if (estatPosta1 == 0 || estatPosta1 == 1 ){
    ...
    if (estatPosta1 == 0 && distancia1 >= 65 && distancia1 <=95 && (millis() - marcaTemps11) > 600 ){    
       if (DEBUG) {
          Serial.println(F("SENSOR 1: Primera Part -- estatPosta1=0"));          
        }
    ...
    }else if (estatPosta1 == 1 && distancia1 >= 65 && distancia1 <=95 ) {
      if (DEBUG) {
          Serial.println(F("SENSOR 1: Segona Part -- estatPosta1=1"));          
      }
      ...
      marcaTemps11=millis();
      enciendeSoloPOSTA1(0,255,255); //MORAT
      estatPosta1=0;
    }  
   }
   //CODI PER ENCENDRE I JUGAR AMB 5 POSTES   
    if (estatPosta1 == 2) {
      if (estatPosta2 == 0 || estatPosta2 == 1 ){
        ...
        if (estatPosta2 == 0 && distancia2 >= 35 && distancia2 <=65 && (millis() - marcaTemps21) > 600){
          enciendeSoloPOSTA2(255,255,0); //GROC
          marcaTemps2 = millis();
          estatPosta2=1;
        }else if (estatPosta2 == 1 && distancia2 >= 35 && distancia2 <= 65 ) {
          if ( (millis() - marcaTemps2) > tempsEspera1 ){
          ...
          marcaTemps21=millis();
          enciendeSoloPOSTA2(0,0,255); //BLAU
          estatPosta2=0;
        }
      }
      ...
      if (estatPosta2 == 2) {
        if (estatPosta3 == 0 || estatPosta3 == 1 ){
          ...
          if (estatPosta3 == 0 && distancia3 >= 5 && distancia3 <=35 && (millis() - marcaTemps31) > 600){
            enciendeSoloPOSTA3(255,255,0); //GROC
            marcaTemps3 = millis();
            estatPosta3=1;
          }else if (estatPosta3 == 1 && distancia3 >= 5 && distancia3 <=35 ) {
            if ( millis() - marcaTemps3 > tempsEspera1 ){
              enciendeSoloPOSTA3(0,255,0); //VERD
              estatPosta3=2;
            }
          }else if (estatPosta3 == 1 && (millis() - marcaTemps3) > 600){
            enciendeSoloPOSTA3(255,0,255); //BLAVET
            estatPosta3=0;
            marcaTemps31 = millis();
          }
        }
        if (estatPosta3 == 2) {
          ...
          if (estatPosta4 == 0 && distancia4 >= 35 && distancia4 <= 65 && (millis() - marcaTemps41) > 600 ){
            enciendeSoloPOSTA4(255,255,0); //GROC
            marcaTemps4 = millis();
            estatPosta4=1;
          }else if (estatPosta4 == 1 && distancia4 >= 35 && distancia4 <= 65 ) {
            if ( millis() - marcaTemps4 > tempsEspera1 ){
              enciendeSoloPOSTA4(0,255,0); //VERD
              estatPosta4=2;
            }
          }else if (estatPosta4 == 1 && (millis() - marcaTemps4) > 600){
            enciendeSoloPOSTA4(0,0,255); //BLAU
            estatPosta4=0;
            marcaTemps41 = millis();
          }
          }
          if (estatPosta4 == 2){
            ...
            if (estatPosta5 == 0 && distancia5 >= 65 && distancia5 <=95 && (millis() - marcaTemps51) > 600 ){
              enciendeSoloPOSTA5(255,255,0); //GROC
              marcaTemps5 = millis();
              estatPosta5=1;
            }else if (estatPosta5 == 1 && distancia5 >= 65 && distancia5 <=95 ) {
              if ( (millis() - marcaTemps5) > tempsEspera1 ){
                enciendeSoloPOSTA5(0,255,0); //VERD
                estatPosta5=2;
              }
            }else if (estatPosta5 == 1 && (millis() - marcaTemps5) > 600){
              enciendeSoloPOSTA5(0,255,255); //MORAT
              estatPosta5=0;
              marcaTemps51 = millis();
            }
          }//EstatPosta4 == 2
    ...
    if (estatPosta1 == 2 && estatPosta2 == 2 && estatPosta3 == 2 && estatPosta4 == 2 && estatPosta5 == 2) {
      client.publish("sala1/casco3","on"); //Envia el event per al sonido i per al pisto
      enciendeSoloCASCO(0,255,0);
      if (DEBUG) {
          Serial.println(F("provaSensorPostes FINAL: Tots els estatPosta = 2 "));          
        }
...
//Funció per realitzar la prova3 situar-se davant de cada posta 5 segs a una distancia que depén del color de la posta
//5-35 COLOR(175,175,0)  35-65 COLOR(0,175,175) 65-95 COLOR(175,0,175)
/*
void prova3b() {
  if (DEBUG2) {
      Serial.println(F("Entra a la funció prova3")); 
  ...
  if (estatPosta5 == 0) {
    enciendeSoloPOSTA5(175,175,0);
    enciendeSoloPOSTA4(175,0,175);
    enciendeSoloPOSTA3(0,175,175);
    enciendeSoloPOSTA2(175,0,175);
    enciendeSoloPOSTA1(175,175,0);
  }
  //INICI SENSOR POSTA5
   if (estatPosta5 == 0 || estatPosta5 == 1 ){
    long distancia5=0;    
    miDistancia5.insertar(sonar[4].convert_cm(sonar[4].ping_median(4)));    
    if (miDistancia5.valida()){
      distancia5 = miDistancia5.media();    
    ...
    if (estatPosta5 == 0 && distancia5 >= 10 && distancia5 <= 30 ){    
       if (DEBUG) {
          Serial.println(F("SENSOR 5: Primera Part -- estatPosta1=0"));          
        }
      
      enciendeSoloPOSTA5(255,100,0); //TARONJA
      marcaTemps5 = millis();
      estatPosta5=1;      
      
    }else if (estatPosta5 == 1 && distancia5 >= 10 && distancia5 <=30 ) {
      if (DEBUG) {
          Serial.println(F("SENSOR 5: Segona Part -- estatPosta1=1"));          
      }
     
      if ( (millis() - marcaTemps5) > tempsEspera1 ){
        if (DEBUG) {
          Serial.println(F("SENSOR 5: Tercera Part -- Pasen 2 segs -- estatPosta1=2"));          
        }
      
        enciendeSoloPOSTA5(0,255,0); //VERD
        estatPosta5=2;
      }
    }else if (estatPosta5 == 1 && (millis() - marcaTemps5) > 500){
      if (DEBUG) {
        Serial.println(F("SENSOR 5: Quarta Part -- torna estatPosta1=0"));          
      } 
      enciendeSoloPOSTA5(175,175,0); //COLOR(175,175,0)
      estatPosta5=0;
    }  
   }
    //INICI SENSOR 3
    if (estatPosta5 == 2) {
      if (estatPosta3 == 0 || estatPosta3 == 1 ){
        ...
        if (estatPosta3 == 0 && distancia3 >= 50 && distancia3 <= 70){
          enciendeSoloPOSTA3(255,100,0); //TARONJA
          marcaTemps3 = millis();
          estatPosta3=1;
        }else if (estatPosta3 == 1 && distancia3 >= 50 && distancia3 <= 70 ) {
          if ( (millis() - marcaTemps3) > tempsEspera1 ){
            enciendeSoloPOSTA3(0,255,0); //VERD
            estatPosta3=2;
          }
        }else if (estatPosta3 == 1 && (millis() - marcaTemps3) > 500){
          enciendeSoloPOSTA3(0,175,175); //COLOR(0,175,175)        
          estatPosta3=0;
        }
      }
        //INICI SENSOR 1
        if (estatPosta3 == 2) {
          if (estatPosta1 == 0 || estatPosta1 == 1 ){
            ...
            if (estatPosta1 == 0 && distancia1 >= 10 && distancia1 <= 30 ){
              enciendeSoloPOSTA1(255,100,0); //TARONJA
              marcaTemps1 = millis();
              estatPosta1=1;
            }else if (estatPosta1 == 1 && distancia1 >= 10 && distancia1 <= 30 ) {
              if ( millis() - marcaTemps1 > tempsEspera1 ){
                enciendeSoloPOSTA1(0,255,0); //VERD
                estatPosta1=2;
              }
            }else if (estatPosta1 == 1 && (millis() - marcaTemps1) > 500){
              enciendeSoloPOSTA1(175,175,0); //COLOR(175,175,0)
              estatPosta1=0;
            }
          }
        //INICI SENSOR 4
        if (estatPosta1 == 2) {
          if (estatPosta4 == 0 || estatPosta4 == 1 ){
            ...
            if (estatPosta4 == 0 && distancia4 >= 90 && distancia4 <= 110 ){
              enciendeSoloPOSTA4(255,100,0); //TARONJA
              marcaTemps4 = millis();
              estatPosta4=1;
            }else if (estatPosta4 == 1 && distancia4 >= 90 && distancia4 <= 110 ) {
              if ( millis() - marcaTemps4 > tempsEspera1 ){
                enciendeSoloPOSTA4(0,255,0);
                estatPosta4=2;
              }
            }else if (estatPosta4 == 1 && (millis() - marcaTemps4) > 500){
              enciendeSoloPOSTA4(175,0,175); //COLOR(175,0,175)
              estatPosta4=0;
            }
          }
          //INICI POSTA 2
          if (estatPosta4 == 2){
            if (estatPosta2 == 0 || estatPosta2 == 1 ){
              long distancia2=0;
              miDistancia2.insertar(sonar[1].convert_cm(sonar[1].ping_median(4)));            
              if (miDistancia2.valida()){
                distancia2 = miDistancia2.media();
              }
              if (estatPosta2 == 0 && distancia2 >= 90 && distancia2 <= 110 ){
                enciendeSoloPOSTA2(255,100,0); //TARONJA
                marcaTemps2 = millis();
                estatPosta2=1;
              }else if (estatPosta2 == 1 && distancia2 >= 90 && distancia2 <= 110 ) {
                if ( millis() - marcaTemps2 > tempsEspera1 ){
                  enciendeSoloPOSTA2(0,255,0); //VERD
                  estatPosta2=2;
                }
              }else if (estatPosta2 == 1 && (millis() - marcaTemps2) > 500){
                enciendeSoloPOSTA2(175,0,175); //COLOR(175,0,175)
                estatPosta2=0;
              }
            }
          }//EstatPosta4 == 2
    ...
    if (estatPosta1 == 2 && estatPosta2 == 2 && estatPosta3 == 2 && estatPosta4 == 2 && estatPosta5 == 2) {
      client.publish("sala1/casco3","on"); //Envia el event per a OCCA
      if (DEBUG) {
          Serial.println(F("provaSensorPostes FINAL: Tots els estatPosta = 2 "));          
        }
        enciendeTODASPOSTAS(0,0,0);
...
void transicio3(){
  ...
  enciendeSoloPOSTA1(0,255,0);
  enciendeSoloPOSTA2(0,255,0);
  enciendeSoloPOSTA3(0,255,0);
  enciendeSoloCASCO(255,255,255);
  delay(tempsEspera2);
}

 
//Funcio per a realitzar la prova4. SIMON presencial de NUMSIMON4 postes a una distancia variable 3 posicions //10-30  50-70 85-100
void prova4(){
  if (DEBUG2) {
      Serial.println(F("Entra a la funció prova4"));
      Serial.print(F("index="));
      Serial.print(index);
      Serial.print(F(" estatPostaVar[index]="));
      Serial.print(estatPostaVar[index]);
  }
   
  if (estatPostaVar[index] == 0 && primeraPosta)  {
    switch(arrayRandomDist[index]){
      case 0: //65-95
        distMin=65;
        distMax=95;
        enciendePOSTA(arrayPostesVar[index],0,255,255);
        break;
      case 1: //35-65
        distMin=35;
        distMax=65;
        enciendePOSTA(arrayPostesVar[index],0,0,255);
        break;
      case 2: //5-35
        distMin=5;
        distMax=35;
        enciendePOSTA(arrayPostesVar[index],255,0,255);
        break;
    }
    primeraPosta=false;
  }
  //INICI SENSORS
   if (estatPostaVar[index] == 0 || estatPostaVar[index] == 1 ){
    //long distancia=0;    
    //miDistancia.insertar(sonar[arrayPostesVar[index]-1].convert_cm(sonar[arrayPostesVar[index]-1].ping_median(4)));    
    //if (miDistancia.valida()){
    //  distancia = miDistancia.media();    
    //}  
    long distancia= sonar[arrayPostesVar[index]-1].convert_cm(sonar[arrayPostesVar[index]-1].ping_median(6));
    if (DEBUG) {
      Serial.print(F("SENSOR:"));
      Serial.print(index);
      Serial.print(F(" ARRAYPOSTESVAR[index]:"));
      Serial.print(arrayPostesVar[index]);
      Serial.print(F(" ARRAYRANDOMDIST[index]:"));
      Serial.print(arrayRandomDist[index]);
      Serial.print(F(" DISTANCIA:"));
      Serial.print(distancia);
      Serial.println(F(" cms"));
    ...
    if (estatPostaVar[index] == 0 && distancia >= distMin && distancia <= distMax && (millis() - marcaTemps11) > 600 ){    
      if (DEBUG) {
         Serial.println(F("SENSOR : Primera Part -- estatPostaVar=0"));          
      }
      enciendePOSTA(arrayPostesVar[index],255,255,0);    //GROC  
      marcaTemps = millis();
      estatPostaVar[index]=1;      
      
    }else if (estatPostaVar[index] == 1 && distancia >= distMin && distancia <= distMax ) {
      if (DEBUG) {
      ...
      if ( (millis() - marcaTemps) > tempsEspera1 ){
        ...
        enciendePOSTA(arrayPostesVar[index],0,255,0); //VERD
        delay(tempsEspera3);
        estatPostaVar[index]=2;
        index++;        
        marcaTemps=0;
        marcaTemps11=0;
        primeraPosta=true;
    ...
    }else if (estatPostaVar[index] == 1 && (millis() - marcaTemps) > 600){
      if (DEBUG) {
...
bool entra=true;
//Funcio per a realitzar la prova5, l'última. SIMON de NUMSIMON5 postes a una distancia variable 3 posicions //5-35  35-65 65-95
void prova5(){
  if (DEBUG2) {
...
void prova5b(){
  ...
  //INICI SENSORS
   if (estatPostaVar[index1] == 0 || estatPostaVar[index1] == 1 ){
    //long distancia=0;    
    //miDistancia.insertar(sonar[arrayPostesVar[index1]-1].convert_cm(sonar[arrayPostesVar[index1]-1].ping_median(4)));    
    //if (miDistancia.valida()){
      //distancia = miDistancia.media();    
    //}  
    ...
    if (estatPostaVar[index1] == 0 && distancia >= distMin && distancia <= distMax ){    
      ...
      apagaPosta(arrayPostesVar[index]);
...
void estat6(){ //Despres de superar les 5 proves
  ...
  if (!pasa){
    ...
    enciendeSoloPOSTA2(255,255,255);
    delay(delayLeds);  
    enciendeSoloPOSTA1(255,255,255);
    delay(delayLeds);
    for (int i=0;  i < 3; i++){    
      apagaTODAS();
      delay(delayLeds);
...
void loop() {
 ...
 switch(estat){    
    case 1:
      ...
      //PROVA per a permaneixer 5 segs davant de cada posta entre 30 i 50 cms 
      prova1();      
      break;
    case 2:
      if (DEBUG2) {
      ...
      //PROVA SIMON pas a pas, permaneixer 2 segs davant de cada posta entre 30 i 50 cms (2 segs per a cada posta) 
      prova2();
      break;
    case 3:
      if (DEBUG2) {
      ...
      prova3(); //PROVA distancia a la posta variable permaneixer 2 segs davant de cada posta variable 3 posicions
      break;
    case 4:    
      if (DEBUG2) {
    ...
    case 5:
      ...
      if (tempsProva5 > 0 && !primeraVegada && (millis() - tempsProva5) > 50000) {
        errorReinici();
        tempsProva5=0;
      }
      break;
